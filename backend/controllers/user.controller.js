...
const getDailyAvg = async (req, res) => {
  try {
...
const getDailyCalorie = async (req, res) => {
  try {
    ...
    const today = new Date().toISOString().split("T")[0];
    const todayEntry = user.dailyCalories.find((entry) => entry.date === today);

    res.status(200).json({
      calories: todayEntry ? todayEntry.calories : 0, 
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal Server Error" });
...
export { loginUser, signupUser, logoutUser, getDailyAvg, postDailyCalorie, getUser, getDailyCalorie };
